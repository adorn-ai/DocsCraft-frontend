...
export default function Dashboard() {
  ...
  const fetchSubscription = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/paystack/subscription`, {
  ...
  // Fetch repos + subscription on mount
  useEffect(() => {
    fetchRepos();
    fetchSubscription();
  }, []);
  ...
  useEffect(() => {
    const hasPending = repos.some(
      (r) => r.clone_status === "pending" || r.clone_status === "processing"
    );

    // If no pending repos → stop polling
    if (!hasPending) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "processing":
        return <Badge className="bg-blue-500 text-white">Analyzing</Badge>;
      default:
      ...
      case "failed":
        return (
          <p className="text-xs text-red-500">
            Failed to fetch repository. Check URL or permissions.
          </p>
        );
      default:
        return null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {/* Faster UsageStats (no lag) */}
        <div className="mb-6">
          {subscription ? (
            <UsageStats key={subscription.subscribed ? "pro" : "free"} />
          ) : (
                          ...
                          {repo.clone_status !== "completed" && (
                            <TooltipContent>
                              <p className="text-xs">
                                ...
                                {repo.clone_status === "processing" &&
                                  "Analyzing repository structure..."}
                                {repo.clone_status === "failed" &&
                                  "Repository fetch failed. Try again."}
                              </p>
                            </TooltipContent>
                          )}
                        </Tooltip>
