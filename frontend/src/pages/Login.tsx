...
export default function Login() {
  const { user, signInWithGithub, signInWithGoogle, clearMFAFactor } =
    useAuth();
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (event === "SIGNED_IN" && session) {
        ...
        if (verifiedFactor) {
          // User has MFA enabled, need to create a challenge
          ...
          if (challenge && !error) {
            ...
            // MFA setup but challenge failed, continue to dashboard
            navigate("/dashboard");
          }
        } else {
        ...
        <CardHeader className="space-y-1">
          <div className="mb-1 text-center">
            <div className="flex items-center justify-center gap-3 mb-4">
              <img src="/logo.png" alt="GitCrafts" className="h-48 w-48" />
            </div>
          </div>
          <CardTitle className="text-2xl text-center">Welcome back</CardTitle>
          <CardDescription className="text-center">
            Sign in to continue to GitCrafts
          </CardDescription>
        </CardHeader>
